...
input = open("../input/euler8.txt").readlines()
rows = input.map do |r| 
...
p test
p rows_to_columns(test)

def rows_to_right_diagonals(rows)
  limit = rows.length
  diagonals = []
  ((limit * 2) - 1).times do |sum|
    diagonal = []
    (sum + 1).times do |y|
      x = sum - y
     if rows[y] and rows[y][x]
      diagonal << rows[y][x]  
     end
    end
    diagonals << diagonal
...
def rows_to_left_diagonals(rows)
  rows = rows.map do |row|
    row.reverse()
  end
  rows_to_right_diagonals(rows)
end
